 ...
 *  haciendo uso de estructuras dinámicas como colas, listas enlazadas y pilas
 * para manejar la llegada, atención y evolución de los pacientes.
 * */
/* BIBLIOTECAS */
#include <stdbool.h>
#include <stdio.h>
...
int main(int argc, char *argv[]) {
  ...
  if (n_docs == -1) {
    printf("[ERROR]: Hubo un error en la lectura del archivo, compruebe que la "
           "entrada siga el formato:\n");
    printf("\t[cant_medicos]\n");
    printf("\t[cant_pacientes]\n");
    printf("\tP[ID];[edad];[prioridad(1-4)];[tiempo_llegada];[tiempo_atencion];"
           "[prob_empeorar];[prob_mejorar]\n");
  ...
  while (hospital_activo) {
    ...
    while (!colaVacia(pacientes_llegada) &&
           pacientes_llegada->front->paciente->tiempo_llegada <= tick) {
      patient_t *p = desencolar(pacientes_llegada);
      espera_t *cola_dest =
          obtener_cola_prioridad(gestor_colas, p->prioridad_actual);
      encolar(cola_dest, p);
    ...
    while (nodo_c != NULL) {
      ...
      while (actual != NULL) {
        ...
        cola_t *siguiente_seguro =
            actual->next; /* Guardar puntero para iteracion segura */
        ...
        if (p->ultimo_tick_triage != tick) {
          ...
          if (chance < p->prob_empeora && p->prioridad_actual > 1) {
            nueva_prio--;
          } else if (chance > (1.0f - p->prob_mejora) &&
                     p->prioridad_actual < 4) {
            nueva_prio++;
          }
          ...
          if (nueva_prio != p->prioridad_actual) {
            ...
            if (extraido) {
              ...
              printf("[Tick %03d] TRIAGE: P%03d cambia de Prio %d a %d\n", tick,
                     extraido->id, extraido->prioridad_actual, nueva_prio);

              extraido->prioridad_actual = nueva_prio;
              encolar(obtener_cola_prioridad(gestor_colas, nueva_prio),
                      extraido);

              /* Alerta si el paciente tiene varios retriage */
              if (extraido->cambios_triage >= 3) {
                printf("[Tick %03d] ALERTA: P%03d ha cambiado de prioridad %d "
                       "veces!\n",
    ...
    if (tick > 0 && tick % 20 == 0 && pila_rollback->tope != NULL) {
      printf("\n--- ROLLBACK PERIODICO: Revirtiendo ultimos 2 cambios (Tick "
             "%d) ---\n",
             tick);
      for (int r = 0; r < 2 && pila_rollback->tope != NULL; r++) {
        ...
        while (busq_c != NULL) {
          p_revertir =
              extraer_paciente(busq_c->cola_espera, retroceso.id_paciente);
          if (p_revertir)
            break;
          busq_c = busq_c->next;
        }
        ...
        if (p_revertir) {
          ...
          encolar(obtener_cola_prioridad(gestor_colas,
                                         retroceso.prioridad_anterior),
                  p_revertir);
        } else {
          /* Verificar si el paciente fue desencolado */
          printf("[Tick %03d] ROLLBACK OMITIDO: P%03d ya se encuentra en "
                 "atencion o finalizado.\n",
      ...
      if (iter_med->medico.occu) {
        ...
        if (iter_med->medico.curr->tiempo_restante <= 0) {
          printf("[Tick %03d] Medico %d TERMINO atencion de P%03d.\n", tick,
                 iter_med->medico.id_medico, iter_med->medico.curr->id);
      ...
      if (!iter_med->medico.occu) {
        ...
        for (int prio = 1; prio <= 4; prio++) {
          ...
          if (!colaVacia(c_prio)) {
            ...
            printf("[Tick %03d] El medico %d comenzo la atencion del paciente "
                   "P%03d (Prioridad %d).\n",
    ...
    while (nodo_c != NULL) {
      ...
      while (actual != NULL) {
        ...
        if (actual->paciente->tiempo_espera ==
            20) { /* Alerta por espera excesiva */
          printf("[Tick %03d] ALERTA: El paciente P%03d lleva 20 ticks "
                 "esperando en la cola de prioridad %d!\n",
      ...
      if (cont_acumulados >= 10 && tick % 5 == 0) { /* Alerta por acumulacion */
        printf("[Tick %03d] ALERTA: Acumulacion de %d pacientes en la cola de "
               "prioridad %d!\n",
    ...
    while (iter_med != NULL) {
      if (iter_med->medico.occu)
        docs_trabajando = true;
      iter_med = iter_med->next;
    }

    /* Si no quedan pacientes en espera y no hay medicos trabajando finalizará
     * la simulación */
    if (colaVacia(pacientes_llegada) && todas_colas_vacias(gestor_colas) &&
        !docs_trabajando) {
...
float generar_random() { return (float)rand() / (float)RAND_MAX; }
...
bool colaVacia(espera_t *cola) { return (cola->front == NULL); }
...
lista_colas_t *crearListaColas() {
  ...
  for (int i = 4; i >= 1; i--) {
    nodo_lista_cola_t *n =
        (nodo_lista_cola_t *)malloc(sizeof(nodo_lista_cola_t));
...
espera_t *obtener_cola_prioridad(lista_colas_t *lista, int prio) {
  ...
  while (actual != NULL) {
    if (actual->prioridad_asignada == prio)
      return actual->cola_espera;
    actual = actual->next;
  }
  return NULL;
}
...
bool todas_colas_vacias(lista_colas_t *lista) {
  nodo_lista_cola_t *actual = lista->cabeza;
  while (actual != NULL) {
    if (!colaVacia(actual->cola_espera))
      return false;
    actual = actual->next;
  }
  return true;
...
void liberar_lista_medicos(lista_docs_t *lista) {
  ...
  while (actual != NULL) {
    ...
    if (temp->medico.curr != NULL)
      free(temp->medico.curr);
    free(temp);
  }
  free(lista);
}
...
int cargar_pacientes(espera_t *pacientes, const char *filename) {
  ...
  for (int i = 0; i < cant_pacientes; i++) {
    ...
    int leidos = fscanf(file, " P%d;%d;%d;%d;%d;%f;%f", &p->id, &p->edad,
                        &p->prioridad_original, &p->tiempo_llegada,
                        &p->tiempo_atencion, &p->prob_empeora, &p->prob_mejora);

    if (leidos != 7) {
      printf(
          "[ERROR]: formato corrupto en el archivo. Paciente %d (Linea %d).\n",
          i + 1, 3 + i);
      free(p);
      fclose(file);
      return -1;
...
void insertar_historial(historial_t *h, patient_t *p, int tick) {
  nodo_historial_t *nuevo =
      (nodo_historial_t *)malloc(sizeof(nodo_historial_t));
  nuevo->paciente = p;
...
void imprimir_metricas(historial_t *h) {
  ...
  for (int i = 0; i < 4; i++) {
    float prom;
    if (conteo_prio[i] > 0)
      prom = (float)sum_espera[i] / (float)conteo_prio[i];
    else
      prom = 0.0f;
    printf("[Prioridad %d] Atendidos: %d, Espera Promedio: %.2f ticks, Espera "
           "Maxima: %d ticks\n",
  ...
  if (conteo_prio[0] > 0) {
    porcentaje_criticos =
        ((float)criticos_a_tiempo / (float)conteo_prio[0]) * 100.0f;
  } else
    porcentaje_criticos = 0.0f;
...
void imprimir_historial_inverso(historial_t *h) {
  ...
  while (actual != NULL) {
    patient_t *p = actual->paciente;
    printf("Alta en Tick %03d -> P%03d (Prio Orig: %d | Prio Final: %d | "
           "Espera: %d ticks)\n",
           actual->tick_salida, p->id, p->prioridad_original,
           p->prioridad_actual, p->tiempo_espera);
    actual = actual->prev;
  }
  printf("-------------------------------------------------------\n");
}
  ...
  while (doc != NULL) {
    if (doc->medico.occu)
      printf("->  Medico %d: Atendiendo a P%03d (Restante: %d)\n",
             doc->medico.id_medico, doc->medico.curr->id,
             doc->medico.curr->tiempo_restante);
    else
